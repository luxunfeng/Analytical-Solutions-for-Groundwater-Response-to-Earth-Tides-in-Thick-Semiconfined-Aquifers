...
%% new model
    ...
    subplot(2,2,1)
    semilogx(omega_non/sqrt(-1),amplitude1,color{jj},'LineWidth',linewidth)
    hold on
    semilogx(omega_non/sqrt(-1),amplitude2,color2{jj},'LineWidth',linewidth)
    xlabel('$\omega_D:$ Dimensionless frequencies of tidal constituents','FontSize',16,'Interpreter', 'latex', 'FontWeight', 'bold')
    ylabel('$|H(i\omega_D)|$','FontSize',16,'Interpreter', 'latex', 'FontWeight', 'bold')
  
    grid on

    title1 = ["$S = 1, C_D ="+ string(C_D)+"$"];
    title(title1,'FontSize',16,'interpreter','latex', 'FontWeight', 'bold');
    ax = gca;
    set(ax, 'FontSize', 20); 
    ...
    subplot(2,2,3)
    semilogx(omega_non/sqrt(-1),phase1,color{jj},'LineWidth',linewidth)
    hold on
    semilogx(omega_non/sqrt(-1),phase2,color2{jj},'LineWidth',linewidth)
    xlabel('$\omega_D$','FontSize',16,'Interpreter', 'latex', 'FontWeight', 'bold')
    ylabel('$arg(H(i\omega_D))$','FontSize',16,'Interpreter', 'latex', 'FontWeight', 'bold')
    grid on
    ax = gca;
    set(ax, 'FontSize', 20); 
    ax.XAxis.TickLabelInterpreter = 'latex';
 ...
 % error
    ...
    subplot(2,2,2)
    semilogx(omega_non/sqrt(-1),100*abs(amplitude1-amplitude2)./amplitude2,color{jj},'LineWidth',linewidth)
    hold on
    xlabel('$\omega_D$','FontSize',16,'Interpreter', 'latex', 'FontWeight', 'bold')
    ...
    subplot(2,2,4)
    semilogx(omega_non/sqrt(-1),100*abs(phase1 - phase2)./abs(phase2),color{jj},'LineWidth',linewidth)
    hold on
    xlabel('$\omega_D$','FontSize',16,'Interpreter', 'latex', 'FontWeight', 'bold')
...
for i = 1 : length(omega_components_non)
    hold on
    plot([omega_components_non(i),omega_components_non(i)],[0,1],'ko-','LineWidth',1)
...
% legend("$h_D\sqrt{H_D}="+sprintf('%0.3f',criterion(1))+"-Gao\ et\ al.,\ 2020$", ...
%     "$h_D\sqrt{H_D}="+sprintf('%0.3f',criterion(1))+"-new\ model$", ...
%         "$h_D\sqrt{H_D}="+sprintf('%0.3f',criterion(2))+"-Gao\ et\ al.,\ 2020$", ...
%         "$h_D\sqrt{H_D}="+sprintf('%0.3f',criterion(2))+"-new\ model$", ...
%          "$h_D\sqrt{H_D}="+sprintf('%0.3f',criterion(3))+"-Gao\ et\ al.,\ 2020$", ...
%          "$h_D\sqrt{H_D}="+sprintf('%0.3f',criterion(3))+"-new\ model$", ...
%         'Interpreter', 'latex', 'FontWeight', 'bold','box','off','Location','eastoutside')
subplot(2,2,3)
for i = 1 : length(omega_components_non)
    hold on
    plot([omega_components_non(i),omega_components_non(i)],[0,100],'ko-','LineWidth',1)
    text(omega_components_non(i), text_y,tides_name{i},'FontSize',18,'Interpreter','latex','Color','k')
end   
...
for i = 1 : length(omega_components_non)
    hold on
    plot([omega_components_non(i),omega_components_non(i)],[0,20],'ko-','LineWidth',1)
    text(omega_components_non(i), text_y,tides_name{i},'FontSize',18,'Interpreter','latex','Color','k')
